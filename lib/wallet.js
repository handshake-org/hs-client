...
class WalletClient extends Client {
  ...
  async init() {
    this.bind('wallet tx', (details) => {
      this.emit('tx', details);
    });

    this.bind('wallet confirmed', (details) => {
      this.emit('confirmed', details);
    });

    this.bind('wallet unconfirmed', (details) => {
      this.emit('unconfirmed', details);
    });

    this.bind('wallet conflict', (details) => {
      this.emit('conflict', details);
    });

    this.bind('wallet updated', (details) => {
      this.emit('updated', details);
    });

    this.bind('wallet address', (receive) => {
      this.emit('address', receive);
    });

    this.bind('wallet balance', (balance) => {
  ...
  async auth() {
    await this.call('wallet auth', this.password);
